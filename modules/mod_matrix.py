...
Version 1.0 Updated 08/14/2025
"""
# ------------------------------------------------------------------------
import os
import json
    ...
    if "SP" in version.upper():
        ...
        if major_version_match:
            ...
            if major_version == 2:
                ...
                logger.info(f"[MOD MATRIX] Mapped version {version} to MR_MP2")
                return "MR_MP2", creds, "ftp", "telnet", creds.get("host_port", "21")
            elif major_version in [3, 4, 5]:
                creds = credentials_data.get("MR_MP3-5", {}).get("credentials", {})
                logger.info(f"[MOD MATRIX] Mapped version {version} to MR_MP3-5")
                return "MR_MP3-5", creds, "ftp", "telnet", creds.get("host_port", "22")
            elif major_version >= 6:
                creds = credentials_data.get("MR_MP6+", {}).get("credentials", {})
                logger.info(f"[MOD MATRIX] Mapped version {version} to MR_MP6+")
                return "MR_MP6+", creds, "sftp", "ssh", creds.get("host_port", "22")
    creds = credentials_data.get("MR_GP", {}).get("credentials", {})
    logger.info(f"[MOD MATRIX] Mapped version {version} to MR_GP")
    return "MR_GP", creds, "ftp", "telnet", creds.get("host_port", "21")
# ------------------------------------------------------------------------
def load_credentials(config_path: str = "../config") -> tuple[str, str, dict, str, str, str]:
        ...
        with current_file.open('r') as f:
            for line in f:
                if line.strip().startswith("SW_Version="):
                    ...
                    break
        if not version:
            logger.error(f"[MOD MATRIX] No SW_Version Found in {current_file}")
            raise ValueError("No SW_Version Found")
    except FileNotFoundError:
        logger.error(f"[MOD MATRIX] Configuration File Missing {current_file}")
        raise
    logger.info(f"[MOD MATRIX] Detected Version: {version}")
    key_file = config_path / "user.key"
    try:
        with key_file.open('rb') as f:
            key = f.read()
    except FileNotFoundError:
        logger.error(f"[MOD MATRIX] Key Not Found {key_file}")
        raise
    try:
        fernet = Fernet(key)
    except Exception as e:
        logger.error(f"[MOD MATRIX] Invalid Key {str(e)}")
        raise
    ...
    try:
        ...
        logger.error(f"[MOD MATRIX] Encrypted Credentials Missing {enc_file}")
        raise
    try:
        decrypted_data = fernet.decrypt(encrypted_data)
        credentials_data = json.loads(decrypted_data.decode())
    except Exception as e:
        logger.error(f"[MOD MATRIX] Decrypt Credentials {str(e)}")
        raise
    version_key, credentials, file_protocol, term_protocol, host_port = map_version_to_credentials(version, credentials_data)
   
    if not credentials:
        logger.error(f"[MOD MATRIX] Missing Credentials {version}")
        raise ValueError(f"Missing Credentials {version}")
    logger.info(f"[MOD MATRIX] Successfully Loaded {version_key}")
...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"[MOD MATRIX] Version: {version}")
        logger.info(f"[MOD MATRIX] Version Key: {version_key}")
        logger.info(f"[MOD MATRIX] File Protocol: {file_proto}")
        logger.info(f"[MOD MATRIX] Terminal Protocol: {term_proto}")
    except Exception as e:
        logger.error(f"[MOD MATRIX] Script failed: {str(e)}")
        raise
# ------------------------------------------------------------------------