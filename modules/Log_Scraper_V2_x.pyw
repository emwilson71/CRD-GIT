...
---------------------------------------------------------------------------------------------
JSmyser
Version 2.84 Updated 02/04/26
---------------------------------------------------------------------------------------------
"""
VERSION = "Log_Scraper_V2_84"
...
class TextFileSearchGUI:
    def __init__(self, master):
        ...
        # 1. Try to get folder path from command line
        folder_from_args = None
        if len(sys.argv) > 1:
            possible_path = sys.argv[1].strip('"')   # remove quotes if user added them
            if os.path.exists(possible_path) and os.path.isdir(possible_path):
                folder_from_args = possible_path
            else:
                print(f"Warning: Provided path does not exist: {possible_path}")

        # 2. Default value: command line > saved preference > empty
        default_path = (
            folder_from_args
            or self.preferences.get('path', '')   # assuming self.preferences is a dict
            or ''
        )
    ...
    ## START OF ROW 0, Tools and Path Entry.
        ...
        self.path_combo = self.create_context_menu(ttk.Combobox(path_frame, textvariable=self.path_var, state="normal", width=38), is_path_combo=True)
...
#### GRAPH RESULTS FROM MAIN. ------------------------------------------------------------------------
    ...
    ## Parse output for terms, examples, and date range
        ...
        def extract_visartconditionhistory2(lines, checked_terms):
            data = {}
            examples = {}
            term_units = {}
            unchecked_terms = set()
            column_names = [
                "File-Date", "Date", "Plane", "Status", "S/N", "Signal", "Signal-SD", "Noise", "Noise-SD", "DC-Noise",
                "Uniformity", "RF", "RF-Scale", "RG", "CF", "Helium", "helium.dat", "shieldTemp.dat", "TGC",
                "M-Pressure", "Pwr", "C_Drive_Total", "C_Drive_Avail", "D_Drive_Total", "D_Drive_Avail",
                "E_Drive_Total", "E_Drive_Avail", "UserAbort", "ProcessAbort"
            ]
            graphable_terms = set(column_names) - set(["File-Date", "Date", "Plane", "Status"])
            graphable_terms.add("RF Level")
            checked_terms = list(set(checked_terms + column_names + ["RF Level"]))
            temp_data = {term: [] for term in graphable_terms}
            known_units = {
                "RF": "dB",
                "RF Level": "dB",
            ...
            for term, unit in known_units.items():
            ...
            for line in lines:
                ...
                for i, (value, term) in enumerate(zip(values, column_names)):
                    ...
                    if term not in examples:
                        examples[term] = f"{term} : {value}"
                    if term not in graphable_terms:
                        continue
                    ...
                    if term == "RF":
                        ...
                        rf_scale = float_value
                    if term not in data:
                        data[term] = []
                    data[term].append((date, float_value, current_file))
                    if term in graphable_terms:
                        temp_data[term].append(float_value)
                    if term not in term_units:
                        match_str = match.group()
                        unit = value.replace(match_str, '').strip()
                        term_units[term] = unit
                if rf is not None and rf_scale is not None:
                    term = "RF Level"
                    if term in checked_terms:
                        rf_level = 10 * math.log10(10 ** ((rf - 63) / 10) * (rf_scale / 32767) ** 2) + 63
                        rf_level = round(rf_level, 3)
                        if term not in data:
                            data[term] = []
                        data[term].append((date, rf_level, current_file))
                        temp_data[term].append(rf_level)
                        if term not in examples:
                            examples[term] = f"{term} : {rf_level}"
           
            unchecked_terms = set(["RF", "RF-Scale", "C_Drive_Total", "C_Drive_Avail", "D_Drive_Total", "D_Drive_Avail", "E_Drive_Total", "E_Drive_Avail"])
            return data, examples, term_units, unchecked_terms
        # End of extract_visartconditionhistory2

        def extract_coolingcabi(lines, checked_terms):
            data = {}
            examples = {}
            term_units = {}
            unchecked_terms = set()
            ...
            key_to_term = {
                "C": "Control mode",
                "dp": "Differential pressure (Pso – Psi)",
                "Pso": "Pressure secondary outlet",
                "Psi": "Pressure secondary inlet",
                "Yp": "Controller output pump speed",
                "Ym": "Controller output mixing valve",
                "Tso": "Temperature secondary output",
                "Tso2": "Temperature secondary output 2",
                "Tpi": "Temperature primary inlet",
                "Tpo": "Temperature primary outlet",
                "Fpo": "Flow primary outlet",
                "Pva": "Pressure Vacuum pump",
                "Tpo2": "Temperature primary outlet circuit 2",
                "Fpo2": "Flow primary outlet circuit 2",
                "Ttr": "Temperature technical room",
                "Htr": "Relative humidity technical room",
                "Vlv": "Position of standby valve",
            }

            term_to_scale = {
                "Control mode": 1,
                "Differential pressure (Pso – Psi)": 0.01,
                "Pressure secondary outlet": 0.01,
                "Pressure secondary inlet": 0.01,
                "Controller output pump speed": 0.1,
                "Temperature secondary output": 0.1,
                "Controller output mixing valve": 0.1,
                "Temperature secondary output 2": 0.1,
                "Temperature primary inlet": 0.1,
                "Temperature primary outlet": 0.1,
                "Flow primary outlet": 1,
                "Pressure Vacuum pump": 0.1,
                "Temperature primary outlet circuit 2": 0.1,
                "Flow primary outlet circuit 2": 1,
                "Temperature technical room": 0.1,
                "Relative humidity technical room": 0.1,
                "Position of standby valve": 0.1,
            }

            term_to_unit = {
                "Control mode": "",
                "Differential pressure (Pso – Psi)": "bar",
                "Pressure secondary outlet": "bar",
                "Pressure secondary inlet": "bar",
                "Controller output pump speed": "%",
                "Temperature secondary output": "°C",
                "Controller output mixing valve": "%",
                "Temperature secondary output 2": "°C",
                "Temperature primary inlet": "°C",
                "Temperature primary outlet": "°C",
                "Flow primary outlet": "l/min",
                "Pressure Vacuum pump": "kPa",
                "Temperature primary outlet circuit 2": "°C",
                "Flow primary outlet circuit 2": "l/min",
                "Temperature technical room": "°C",
                "Relative humidity technical room": "%",
                "Position of standby valve": "%",
            }

            column_names = list(key_to_term.values())
            graphable_terms = set(column_names) - {"Control mode"}
            checked_terms = list(set(checked_terms + column_names))
            temp_data = {term: [] for term in graphable_terms}
            ...
            for line in lines:
                ...
                date_part = header_tokens[1]   # e.g. 2025/08/11
                time_part = header_tokens[2]   # e.g. 00:14:55
                ...
                raw_data = {}
                for token in data_content.split():
                    if ":" not in token:
                        continue
                ...
                for raw_key, raw_value in raw_data.items():
                    if raw_key not in key_to_term or raw_value == "---":
                        continue

                    term = key_to_term[raw_key]
                    if term not in checked_terms:
                        continue

                    if term not in examples:
                        examples[term] = f"{term} : {raw_value}"

                    if term not in graphable_terms:
                        continue

                    try:
                        float_value = float(raw_value) * term_to_scale[term]
                    except ValueError:
                        continue
                    ...
                    data[term].append((date, float_value, current_file))
                    temp_data[term].append(float_value)
                    term_units[term] = term_to_unit[term]

            return data, examples, term_units, unchecked_terms
        # End of extract_coolingcabi
        ...
        def extract_data(checked_terms):
            ...
            #    r'\.sigusr1$': extract_acqsts, # removed because it doesnt' have dates. Saving for later.  
                r'gcoiltemp.*\.log$': extract_gcoiltemp,
                r'visartconditionhistory2\.csv$': extract_visartconditionhistory2,
                r'.*coolingcabi\.txt$': extract_coolingcabi,
            }
            data = {}
            examples = {}
            term_units = {}
...
#### Right Click Control for Main page Entry Boxes and Log_Viewer ------------------------------------------------------
    ...
    def create_context_menu(self, widget, is_path_combo=False):
        ...
        if is_path_combo:
            ...
            def open_current_folder():
                ...
                if not folder:
                    # Optional: give feedback instead of silently failing
                    self.root.bell()  # small beep
                    return
                ...
                if os.path.exists(folder):
                    try:
                        ...
                        messagebox.showerror("Cannot Open", f"Failed to open folder:\n{e}")
                else:
                    messagebox.showinfo("Not Found", "The current folder path does not exist")

            context_menu.add_command(label="Open Folder", command=open_current_folder)
...
#### Quick Search Dropdown and Manage Quick Search. -----------------------------------------------
    ...
    def update_quick_search_dropdown(self):
        ...
        self.quick_search_dropdown.config(height=30)  # Doubled from default (typically ~10) to 20
...
#                return None
#            THIS RETURN IS CAUSING NESTED TAR'S TO NOT EXTRACT.
#            PROBLEM IS WITHOUT IT, IT ONLY SEARCHES IN THE TAR EXTRACTIONS
#            AND DOES NOT APPEND.
...
# THIS FILES.CLEAR() IS CAUSING THE FIRST FILES LIST TO DELETE 
# WHEN HNA IS CALLED AGAIN FOR NESTED ARCHIVES. 
...
#                        print(f"[HNA7] handle_nested_archives: Stopped at compressed file {nested_path} Nested ext. {nested_ext}")
    ...
    def search_completed(self, has_results, error_files):
        ...
        if self.searching:
            ...
            if not has_results:
                ...
                if self.date_filter_var.get():
                    self.output.insert(tk.END, " No results found.\n FYI,your filter by date checkbox is checked.")
                else:
                    self.output.insert(tk.END, " No results found.\n")
            else:
                ...
                if output_line.startswith(">>>> ") and output_line.endswith(" <<<<"):    
                    ...
                    for parse_file in self.result_files:
                        ...
                        if base_name == current_file: # or base_name.startswith(current_file) or current_file.startswith(base_name.split()[0]):
                            # THIS COMMENTED SECITON ABOVE WAS CAUSING acqman-status FILE TO NOT HIGHLIGHT LINES. 
