...
def read_credentials(ini_data=None):
    ...
    is_pre_v6 = False
    if ini_data:
        parsed_data = parse_ini_data(ini_data)
        if parsed_data:
            if parsed_data.get("ModalityOSVersion") or parsed_data.get("ModalitySystemVersion1"):
                is_win10 = True
            raw_version = parsed_data.get("RawVersion", "")
            if raw_version:
                try:
                    version_num = float(raw_version.split('*')[0].lstrip('VSM '))
                    if version_num < 6.0:
                        is_pre_v6 = True
                except (ValueError, IndexError):
                    logger.warning(f"[CONNECTVPN] Could Not Parse {raw_version}", extra={"tag": tag})
    if os.path.exists(enc_file_path) and os.path.exists(key_file_path):
        ...
        if creds_dict:
            logger.debug(f"[CONNECTVPN] user.enc contents: {creds_dict.keys()}", extra={"tag": tag})
            credential_sets = []
            ...
            if credential_sets:
                if is_win10 or not is_pre_v6:
                    return [cs for cs in credential_sets if cs["source"] == "SP_WIN10"] + [cs for cs in credential_sets if cs["source"] != "SP_WIN10"]
...
def establish_ssh_connection(spip, remote_path, creds_dict):
    ...
    for credentials in creds_dict:
        ...
        try:
            ...
            logger.error(f"[CONNECTVPN] Invalid Port: {credentials['port']} for {credentials['source']}, using default", extra={"tag": tag})
            port = 22 if protocol == "SFTP" else 21
        source = credentials["source"]
        logger.info(f"[CONNECTVPN] Attempting Connection With {source} Credentials: {spip} as {username} using {protocol}", extra={"tag": tag})
        ...
        if protocol == "SFTP":
            ...
            try:
                ...
                logger.info(f"[CONNECTVPN] SFTP Connection Established with {source} to {spip}:{port}", extra={"tag": tag})
                ...
                logger.warning(f"[CONNECTVPN] SFTP failed with {source} on port {port}: {sftp_error}", extra={"tag": tag})
                try:
                    ...
                    if check_error or "spsite.ini" not in check_output.lower():
                        logger.error(f"[CONNECTVPN] File {remote_path} Not Found: {check_error or 'File not listed'}", extra={"tag": tag})
                        raise FileNotFoundError(f"File {remote_path} not found")
                   
                    command = f"type \"{remote_path}\""
                    logger.info(f"[CONNECTVPN] Executing SSH Command: {command}", extra={"tag": tag})
                    stdin, stdout, stderr = client.exec_command(command, timeout=10)
                    error_output = stderr.read().decode('utf-8').strip()
                    if error_output:
                        logger.error(f"[CONNECTVPN] SSH command error: {error_output}", extra={"tag": tag})
                        raise RuntimeError(f"SSH command error: {error_output}")
                   
                    ini_output = stdout.read().decode('utf-8').strip()
                    if not ini_output:
                        logger.error(f"[CONNECTVPN] Empty File {remote_path}", extra={"tag": tag})
                        raise ValueError(f"Empty File {remote_path}")
                    logger.debug(f"[CONNECTVPN] Full INI Data Received via SSH:\n{ini_output}", extra={"tag": tag})
                    return ini_output
               
                except (paramiko.SSHException, socket.timeout, paramiko.AuthenticationException, OSError) as ssh_error:
                    logger.warning(f"[CONNECTVPN] SSH Failed {source} on Port {port}: {ssh_error}", extra={"tag": tag})
                    continue
                finally:
                    if sftp:
                        sftp.close()
            ...
            try:
                ...
                logger.info(f"[CONNECTVPN] FTP Connection Established with {source} to {spip}:{port}", extra={"tag": tag})
                ...
                if ftp_dir:
                    ftp.cwd(ftp_dir)
                    logger.info(f"[CONNECTVPN] Changed to directory: {ftp_dir}", extra={"tag": tag})
                ...
                for encoding in encodings:
                    try:
                        ...
                        logger.debug(f"[CONNECTVPN] Failed to Decode With {encoding}", extra={"tag": tag})
                        continue
                if not ini_output:
                    logger.error(f"[CONNECTVPN] Could Not Decode File {remote_path}", extra={"tag": tag})
                    raise ValueError(f"Could Not Decode File Content: {remote_path}")
                logger.debug(f"[CONNECTVPN] Full INI Data Received via FTP:\n{ini_output}", extra={"tag": tag})
                return ini_output
           
            except ftplib.all_errors as ftp_error:
                logger.warning(f"[CONNECTVPN] FTP Failed With {source} on Port {port} {ftp_error}", extra={"tag": tag})
                continue
            finally:
                if ftp:
                    ftp.quit()
...
def parse_ini_data(ini_data, sid=None, existing_modality=None):
    ...
    try:
        ...
        modality = None
        key1 = None
        if 'CT' in config.sections() and config.get('CT', 'KeyMax', fallback='') == '2':
            modality = 'CT'
            key1 = config.get('CT', 'Key1', fallback='')
            if key1 and key1 in config.sections():
                key1_section = key1
            if 'MR' in config.sections() and config.get('MR', 'KeyMax', fallback='') != '0':
                logger.warning(f"[CONNECTVPN] Expected CT KeyMax=2")
        elif 'MR' in config.sections() and config.get('MR', 'KeyMax', fallback='') == '1':
            modality = 'MR'
            key1 = config.get('MR', 'Key1', fallback='')
            if key1 and key1 in config.sections():
                key1_section = key1
                logger.info(f"[CONNECTVPN] MR Key1 Section: {key1_section}", extra={"tag": tag})
            if 'CT' in config.sections() and config.get('CT', 'KeyMax', fallback='') != '0':
                logger.warning(f"[CONNECTVPN] Expected CT KeyMax=0 for MR, found: {config.get('CT', 'KeyMax')}", extra={"tag": tag})

        if not modality:
            modality = existing_modality or "Unknown"
            logger.warning(f"[CONNECTVPN] No valid modality determined, using: {modality}", extra={"tag": tag})
        if key1 and key1 in config.sections():
            data.update({
                "MachineName": config.get(key1, 'MachineName', fallback=''),
                "Modality": config.get(key1, 'Modality', fallback=modality),
            ...
            logger.error(f"[CONNECTVPN] Key1 Section [{key1}] Not Found", extra={"tag": tag})
        if not data["MachineName"] or not data["HospName"]:
            logger.info("[CONNECTVPN] Falling Back to Regex Parsing", extra={"tag": tag})
            in_mr_section = False
            modality = None
            key1 = None
            for line in ini_data.splitlines():
                ...
                if in_mr_section and re.match(r'\[.*\]', line):
                    ...
                    continue
                if in_mr_section:
                    if re.match(r'KeyMax\s*=\s*1', line, re.IGNORECASE):
                        modality = 'MR'
                    if re.match(r'Key1\s*=\s*(.+)', line, re.IGNORECASE):
                        key1 = re.match(r'Key1\s*=\s*(.+)', line, re.IGNORECASE).group(1).strip()
                        logger.info(f"[CONNECTVPN] Regex Modality: {modality}, Key1: {key1}", extra={"tag": tag})
            if key1:
                in_spsite_section = False
                for line in ini_data.splitlines():
                    line = line.strip()
                    if re.match(r'\[SPSiteInfo\]', line, re.IGNORECASE):
                        in_spsite_section = True
                        continue
                    if in_spsite_section and re.match(r'\[.*\]', line):
                        in_spsite_section = False
                        continue
                    if in_spsite_section:
                        match = re.match(r'HospName\s*=\s*(.+)', line, re.IGNORECASE)
                        if match:
                            data["HospName"] = match.group(1).strip().replace("PreInstall:", "")
                            continue
                in_key_section = False
                for line in ini_data.splitlines():
                    line = line.strip()
                    if re.match(rf'\[{re.escape(key1)}\]', line):
                        in_key_section = True
                        continue
                    if in_key_section and re.match(r'\[.*\]', line):
                        in_key_section = False
                        continue
                    if in_key_section:
                        match = re.match(r'MachineName\s*=\s*(.+)', line, re.IGNORECASE)
                        if match:
                            data["MachineName"] = match.group(1).strip()
                            continue
                        match = re.match(r'Modality\s*=\s*(.+)', line, re.IGNORECASE)
                        if match:
                            data["Modality"] = match.group(1).strip()
                            continue
                        match = re.match(r'ModalityOSVersion\s*=\s*(.+)', line, re.IGNORECASE)
                        if match:
                            data["ModalityOSVersion"] = match.group(1).strip()
                            continue
                        match = re.match(r'ModalitySystemVersion1\s*=\s*(.+)', line, re.IGNORECASE)
                        if match:
                            data["ModalitySystemVersion1"] = match.group(1).strip()
                            continue
                        match = re.match(r'SmVersion\s*=\s*(.+)', line, re.IGNORECASE)
                        if match:
...
class QueryThread(QThread):
    ...
    def run(self):
        ...
        if self.isInterruptionRequested():
            logger.info("[CONNECTVPN] QueryThread Interrupted Before Starting", extra={"tag": tag})
            return
        try:
            self.update_message.emit("Collecting Data From VPN Server")
            tams_ips = get_ips(self.sid)
            if not tams_ips:
                logger.error(f"[CONNECTVPN] No VPN Data Found For SID: {self.sid}", extra={"tag": tag})
            ...
            display_ip = tams_ips.get('CT Display', {}).get('tams_ip', '')
            firewall_type = tams_ips.get('vpn_type', '')
            port = tams_ips.get('port', '10') 
            if not sp_ip:
                logger.warning(f"[CONNECTVPN] No SP IP Found For SID: {self.sid}", extra={"tag": tag})
                sp_ip = scan_ip or infinix_ip or sm_ip
                if not sp_ip:
                    logger.error(f"[CONNECTVPN] No IP Found for SID: {self.sid}", extra={"tag": tag})
            ...
            ini_output = establish_ssh_connection(spip=sp_ip, remote_path=remote_path, creds_dict=self.creds_dict)
            if ini_output:
                ...
                return
            ini_data = parse_ini_data(ini_output, sid=self.sid, existing_modality=None)
            if not ini_data:
                logger.info("[CONNECTVPN] No INI Data Parsed", extra={"tag": tag})
                ini_data = {"HospName": "", "ModalitySystemVersion1": "", "MachineName": "", "Modality": "Unknown", "RawVersion": ""}
            modality = ini_data.get("Modality", "").upper()
            if modality not in ["MR", "CT"]:
                modality = "CT" if scan_ip or display_ip else "MR" if sm_ip or infinix_ip else "XR"
            host_ip = ""
            if modality == "CT":
                host_ip = scan_ip if scan_ip else ini_data.get("host_ip", "")
            else:
                host_ip = sm_ip if sm_ip else infinix_ip if infinix_ip else ini_data.get("host_ip", "")
            logger.info(f"Host IP set to: {host_ip}", extra={"tag": tag})
            sw_version_value = ini_data.get("ModalitySystemVersion1", "") or ini_data.get("RawVersion", "")
            data = {
                "sp_ip": str(sp_ip),
                "host_ip": str(host_ip) if host_ip else "",
                "TunnelType": str(firewall_type),
                "display_ip": str(display_ip) if modality == "CT" and display_ip else "",
                "HospName": str(ini_data.get("HospName", "")),
                "sw_version": str(sw_version_value),
                "machine": str(ini_data.get("MachineName", "")),
                "Modality": str(modality),
                "sid": str(self.sid),
                "port": str(port)
            }
            logger.info(f"[CONNECTVPN] SID {self.sid}: {json.dumps(data, indent=2)}", extra={"tag": tag})
...
def get_ips(sid):
    ...
    try:
        ...
        query = """
            ...
            AND snsVpnDevice.device IN ('SM', 'Infinix DFP', 'CT Scan', 'CT Display', 'InnerVision SP', 'CT Innervision SP')
        """
...
def save_to_json(sid, main_app):
    ...
    try:
        ...
        entry = {
            ...
            "display_ip": [ip.strip() for ip in main_app.display_ip_edit_box.text().split(",") if ip.strip()] if hasattr(main_app, "display_ip_edit_box") and isinstance(main_app.display_ip_edit_box, QLineEdit) else [],
            ...
            "modality": [m.strip() for m in main_app.edit_boxes.get("Modality", QLineEdit()).text().split(",") if m.strip()] if hasattr(main_app, "edit_boxes") and isinstance(main_app.edit_boxes.get("Modality"), QLineEdit) else [],
...
def query_sid(sid, main_app=None, creds_dict=None):
        ...
        if main_app:
            try:
                if hasattr(main_app, 'sp_ip_edit_box') and isinstance(main_app.sp_ip_edit_box, QLineEdit):
                    ...
                    logger.info(f"[CONNECTVPN] Updated sp_ip_edit_box with: {data['sp_ip']}", extra={"tag": tag})
                if hasattr(main_app, 'sm_ip_edit_box') and isinstance(main_app.sm_ip_edit_box, QLineEdit):
                    ...
                    logger.info(f"[CONNECTVPN] Updated sm_ip_edit_box with: {data['host_ip']}", extra={"tag": tag})
                if hasattr(main_app, 'scan_ip_edit_box') and isinstance(main_app.scan_ip_edit_box, QLineEdit):
                    ...
                    logger.info(f"[CONNECTVPN] Updated scan_ip_edit_box with: {data['host_ip']}", extra={"tag": tag})
                if hasattr(main_app, 'display_ip_edit_box') and isinstance(main_app.display_ip_edit_box, QLineEdit):
                    main_app.display_ip_edit_box.setText(data["display_ip"])
                    logger.info(f"[CONNECTVPN] Updated display_ip_edit_box with: {data['display_ip']}", extra={"tag": tag})
                if hasattr(main_app, 'tunnel_edit_box') and isinstance(main_app.tunnel_edit_box, QLineEdit):
                    ...
                    logger.info(f"[CONNECTVPN] Updated tunnel_edit_box with: {data['TunnelType']}", extra={"tag": tag})
                if 'sw_version' in main_app.edit_boxes and isinstance(main_app.edit_boxes['sw_version'], QLineEdit):
                    ...
                    logger.info(f"[CONNECTVPN] Updated sw_version with: {data['sw_version']}", extra={"tag": tag})
                if 'machine' in main_app.edit_boxes and isinstance(main_app.edit_boxes['machine'], QLineEdit):
                    ...
                    logger.info(f"[CONNECTVPN] Updated machine with: {data['machine']}", extra={"tag": tag})
    ...
    if thread.isRunning():
        ...
        thread.wait(1000)
        if thread.isRunning():
            logger.warning("[CONNECTVPN] QueryThread Did Not Terminate Properly", extra={"tag": tag})
            thread.terminate()
            thread.wait()
    return None
# ------------------------------------------------------------------------
