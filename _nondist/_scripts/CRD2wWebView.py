        ...
        for sub_name in [" VPN ", " LOCAL "]:
            ...
            if sub_name.strip() == "VPN":
                ...
                header_widget.setStyleSheet(
                    "background-color: #202020; border-bottom: 1px solid #444444;"
                )
                ...
                # Create the browser FIRST.
                self.local_browser = QWebEngineView()
                self.local_browser.setStyleSheet(Styles.WEB_VIEW_WIDGET)
                self.local_browser.setPage(CustomWebEnginePage(self.local_browser))
                ...
                # Diagnostics for the local browser.
                self.local_browser.loadStarted.connect(
                    lambda: logger.info("[CRD UI] Local browser: load started")
                )
                self.local_browser.loadProgress.connect(
                    lambda progress: logger.debug(
                        f"[CRD UI] Local browser: load progress: {progress}%"
                    )
                )
                self.local_browser.loadFinished.connect(
                    lambda ok: logger.info(
                        f"[CRD UI] Local browser: finished={ok}, "
                        f"url={self.local_browser.url().toString()}"
                    )
                )
                ...
                self.btn_back.clicked.connect(self.local_browser.back)
                ...
                self.btn_forward.clicked.connect(self.local_browser.forward)
                ...
                self.btn_refresh.clicked.connect(self.local_browser.reload)
                ...
                self.local_browser.urlChanged.connect(
