...
def setup_logging():
    ...
    log_base = os.path.join(log_dir, "module.log")
    ...
    for log_file in log_files:
        try:
            ...
            logging.warning(f"Failed to Delete {log_file} {str(e)}")
    
    handler = RotatingFileHandler(log_base, maxBytes=5 * 1024 * 1024, backupCount=5)
...
def parse_key_value_file(file_path):
    ...
    try:
        ...
        if not os.access(file_path, os.R_OK):
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Error")
            msg_box.setText(f"No Read Permission for {file_path}")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
        ...
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Error")
        msg_box.setText(f"Failed to Decode '{file_path}' as UTF-8: {e}")
        msg_box.setIcon(QMessageBox.Critical)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
        sys.exit(1)
        ...
        msg_box.exec()
        sys.exit(1)
    logging.debug(f"Loaded config from {file_path}: {config}")
    return config
...
def read_credentials(credential_path=None):
    ...
    if not os.path.exists(key_path):
        ...
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Error")
        msg_box.setText(f"Fernet Key Not Found: {key_path}")
        msg_box.setIcon(QMessageBox.Critical)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
        sys.exit(1)
    if not os.path.exists(credential_path):
        ...
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Error")
        msg_box.setText(f"Credentials File Not Found: {credential_path}")
        msg_box.setIcon(QMessageBox.Critical)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
        sys.exit(1)
    try:
        ...
        try:
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Error")
            msg_box.setText(f"Invalid Fernet Key in '{key_path}': {e}")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
        ...
        logging.error(f"Invalid JSON Decrypted '{credential_path}': {e}")
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Error")
        msg_box.setText(f"Invalid JSON Decrypted '{credential_path}': {e}")
        msg_box.setIcon(QMessage.Critical)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
        sys.exit(1)
    except Exception as e:
        logging.error(f"Failed to Decrypt '{credential_path}': {e}")
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Error")
        msg_box.setText(f"Failed to Decrypt '{credential_path}': {e}")
        msg_box.setIcon(QMessageBox.Critical)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
        sys.exit(1)
    ...
    try:
        ...
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Error")
        msg_box.setText(f"Error parsing credentials: {e}")
        msg_box.setIcon(QMessageBox.Critical)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
        sys.exit(1)
    ...
    for key in expected_keys:
        if not config.get(key):
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Error")
            msg_box.setText(f"Missing '{key}' in credentials file")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
    logging.debug(f"Loaded credentials: {config}")
    return config
# CLASS CONFIGMGR ------------------------------------------------------
class ConfigManager:
    def __init__(self, config_path=None):
        ...
        self.hosp_name = None
        logging.info(f"ConfigManager initialized - sm_ip: {self.sm_ip}, sp_ip: {self.sp_ip}, sys_id: {self.sys_id}, sm_version: {self.sm_version}")
# VALIDATE SID FIRST
        if not self.sys_id:
            logging.error("'SID' Is Missing")
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Configuration Error")
            msg_box.setText("'SID' Is Missing")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
# ADD -000 TO SYS_ID
    ...
    def validate_config(self):
        try:
            ...
            logging.error(f"Invalid IP Address: sm_ip={self.sm_ip}, sp_ip={self.sp_ip}")
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Configuration Error")
            msg_box.setText(f"Invalid IP Addresses: sm_ip={self.sm_ip}, sp_ip={self.sp_ip}")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
        if not self.sys_id or not all(c.isalnum() or c == '-' for c in self.sys_id):
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Configuration Error")
            msg_box.setText(f"Invalid System ID: {self.sys_id}")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
        try:
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Configuration Error")
            msg_box.setText(f"Cannot Write '{self.monitor_directory}': {e}")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
            msg_box.exec()
            sys.exit(1)
    ...
    def run(self):
        try:
            ...
            if exit_status == 0:
                ...
                self.finished.emit(f"[{self.hostname}] {output or 'Command Executed'}")
            else:
...
# WAIT FOR SNAPSHOT TO TRANSFER TO SP
    def run(self):
        try:
...
class SnapshotWorkflowWorker(QObject):
    ...
    def __init__(self, config_manager, credentials):
        ...
        logging.debug(f"SnapshotWorkflowWorker - sm_ip: {self.sm_ip}, sp_ip: {self.sp_ip}, sys_id: {self.config_manager.get_sys_id()}, sm_version: {self.sm_version}")
# ---------------------------------------------------------------------
    def run(self):
...
# SETUP UI
        ...
        self.monitor_directory = self.config_manager.get_monitor_directory()
        self.snapshot_processor = SnapshotProcessor(config_manager)
        logging.debug(f"SnapshotToSP - sm_ip: {self.sm_ip}, sp_ip: {self.sp_ip}, sys_id: {self.config_manager.get_sys_id()}")
...
# ---------------------------------------------------------------------
    def on_workflow_finished(self):
        ...
        msg_box.setText("Workflow Executed OK")
...
# ---------------------------------------------------------------------
    def copy_output_to_clipboard(self):
        ...
        msg_box.setText("Copied to Clipboard Successfully")
        msg_box.setIcon(QMessageBox.Information)
        msg_box.setStyleSheet(MESSAGE_BOX_STYLE)
        msg_box.exec()
# ---------------------------------------------------------------------
    ...
    def closeEvent(self, event):
        if hasattr(self, 'workflow_thread') and self.workflow_thread.isRunning():
            ...
            if self.workflow_thread.isRunning():
                logging.warning("Forcing termination of workflow_thread")
            ...
            try:
                if hasattr(self.workflow_worker, 'sm_thread') and self.workflow_worker.sm_thread.isRunning():
                    ...
                    if self.workflow_worker.sm_thread.isRunning():
                        logging.warning("Forcing termination of sm_thread")
                ...
                if hasattr(self.workflow_worker, 'sftp_thread') and self.workflow_worker.sftp_thread.isRunning():
                    ...
                    if self.workflow_worker.sftp_thread.isRunning():
                        logging.warning("Forcing termination of sftp_thread")
                        self.workflow_worker.sftp_thread.terminate()
                        self.workflow_worker.sftp_thread.wait()
            except Exception as e:
                logging.error(f"Error terminating worker threads: {e}")
        event.accept()
# MAIN -----------------------------------------------------------------
def main():
    setup_logging()
