...
"""
savelog module
emwilson71@yahoo.com 10042025
savelog gp - need to verify path and monitor telnet until file is
created then use ftp. Also Mod Logging
"""
#-----------------------------------------------------------------------
import os
import sys
...
def setup_logging():
    ...
    log_path = os.path.normpath(os.path.join(script_dir, "..", "logs", "savelogGP.log"))
...
class TelnetWorker(QObject):
    ...
    def __init__(self, hostname, username, password, commands, check_dir="C:/tmp", check_file="savelog.zip", timeout=1200):
        super().__init__()
...
#-----------------------------------------------------------------------
    def run(self):
        tn = None
        try:
            self.log.emit(f"Establishing Telnet Connection {self.hostname}:23 As {self.username}", "INFO")
            ...
            while time.time() - start_time < self.timeout:
                self.log.emit(f"Checking {self.check_dir} for {self.check_file}...", "INFO")
                tn.write(f"ls {self.check_dir}\n".encode())
                response = tn.read_until(b"\n", timeout=15).decode().strip()
                output.append(f"[ls {self.check_dir}] {response}")# or 'No output'}")
...
class FTPWorker(QObject):
    ...
    def __init__(self, hostname, username, password, remote_dir, local_dir, sys_id, start_time, search_wait=1200):
        ...
        self.username = username
        self.password = password
        self.remote_dirs = [remote_dir, "tmp", "TMP", "Temp"]
...
#-----------------------------------------------------------------------
    def run(self):
        ftp = None
        try:
            self.log.emit(f"Attempting FTP Connection To {self.hostname}:21 As {self.username}...", "INFO")
            ftp = ftplib.FTP()
            ftp.connect(self.hostname, 21, timeout=10)
            ftp.login(self.username, self.password)
            self.log.emit(f"FTP Connection Established", "INFO")
            

            try:
                root_dir = ftp.pwd()
                self.log.emit(f"FTP Server Root Directory: {root_dir}", "INFO")
                root_files = ftp.nlst()
                self.log.emit(f"Files in Root Directory: {', '.join(root_files) if root_files else 'None'}", "INFO")
            except Exception as e:
                self.log.emit(f"Failed to get FTP root directory: {str(e)}", "WARNING")
            
            savelog_found = False
            start_time = self.start_time
            for remote_dir in self.remote_dirs:
                self.log.emit(f"Monitoring Remote Directory via FTP: {remote_dir}", "INFO")
                while time.time() - start_time < self.search_wait and not self._cancelled:
                    try:
                        ftp.cwd(remote_dir)
                        log_files = ftp.nlst()
                        self.log.emit(f"Files found in {remote_dir}: {', '.join(log_files) if log_files else 'None'}", "INFO")
                        ...
                        if savelog_logs:
                            ...
                            self.log.emit(f"Found {latest_log} in {remote_dir}, size: {curr_size} bytes, checking stability...", "INFO")
                            time.sleep(10)
                            if self._cancelled:
                                break
                            ...
                            if curr_size == new_size and curr_size != 0:
                                savelog_found = True
                                timestamp = datetime.fromtimestamp(time.time()).strftime('%d%m%Y_%HMMSS')
                            ...
                            self.log.emit(f"Waiting for savelog.zip file in {remote_dir}", "INFO")
                    except ftplib.error_perm as e:
                        self.log.emit(f"Directory {remote_dir} not accessible: {str(e)}", "WARNING")
                        break 
                    time.sleep(5)
                if savelog_found or self._cancelled:
                    break
            
            if self._cancelled:
                self.error.emit("Operation cancelled by user")
            elif not savelog_found:
                self.error.emit(f"[{self.hostname}] No stable savelog.zip file found in any directory after timeout")
        
        except ftplib.error_perm as e:
            self.error.emit(f"[{self.hostname}] FTP Authentication Failed: {str(e)}")
        except Exception as e:
            self.error.emit(f"[{self.hostname}] FTP Connection Failed: {str(e)}")
        finally:
            if ftp and ftp.sock is not None:
                try:
                    ftp.quit()
                    self.log.emit("FTP Connection Closed", "INFO")
                except Exception as e:
                    self.log.emit(f"Error closing FTP connection: {str(e)}", "WARNING")
...
#-----------------------------------------------------------------------
    def run(self):
        if self.credentials['term_protocol'] != 'telnet':
            self.error.emit("MR_GP requires Telnet protocol")
            return
        if self.credentials['file_protocol'] != 'ftp':
            self.error.emit("MR_GP requires FTP protocol")
            return
        
        commands = ["csh", "savelog"]
        sm_thread = QThread()
...
class SavelogGPTool(QMainWindow):
    def __init__(self, config_manager):
        super().__init__()
        self.setWindowTitle("SAVELOG GP TOOL")
        ...
        if "SP" in self.sm_version:
            logging.error(f"Invalid version for savelogGP: {self.sm_version}. Expected MR_GP.")
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Error")
            msg_box.setText(f"Invalid version: {self.sm_version}. This tool is for MR_GP only.")
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.exec()
            sys.exit(1)
        ...
        try:
            ...
            if self.credentials['term_protocol'] != 'telnet' or self.credentials['file_protocol'] != 'ftp':
                logging.error(f"Invalid protocols for MR_GP: file_protocol={self.credentials['file_protocol']}, term_protocol={self.credentials['term_protocol']}")
                msg_box = QMessageBox()
                msg_box.setWindowTitle("Error")
                msg_box.setText("MR_GP requires Telnet and FTP protocols")
                msg_box.setIcon(QMessageBox.Critical)
                msg_box.exec()
                sys.exit(1)
        except Exception as e:
...
#-----------------------------------------------------------------------
    def on_workflow_finished(self):
        self.progress.close()
        self.append_log("Savelog Downloaded=", "INFO")
        msg_box = QMessageBox()
...
def main():
    ...
    window = SavelogGPTool(config_manager)
    window.show()
    sys.exit(app.exec())
